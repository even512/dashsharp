...
### Added
- **JDownloader-Kachel (MyJDownloader)** — eine neue Live-Kachel für einen (auch headless/„autarken") JDownloader, angebunden über die MyJDownloader-Cloud-API (`my.jdownloader.org`). Zeigt die laufenden/letzten Downloads als Liste mit Name, Fortschrittsbalken, Speed und ETA (laufende zuerst, fertige zuletzt), den aggregierten Gesamt-Speed samt Status (läuft/pausiert/bereit/offline), Queue-Zahlen (aktiv/wartend/Linkgrabber) sowie fertige Downloads und die verbleibende Gesamtgröße. Die vier Inhaltsblöcke sind über die Kachel-Einstellungen einzeln an-/abschaltbar, die Länge der Download-Liste ist begrenzbar. Konfiguration über die UI (Einstellungen → JDownloader) oder Env (`JDOWNLOADER_EMAIL`/`JDOWNLOADER_PASS`, optional `JDOWNLOADER_DEVICE`/`JDOWNLOADER_APPKEY`). Der Handshake (E-Mail/Passwort → `/my/connect` → Geräte-Aktionen) läuft serverseitig mit HMAC-SHA256-Signatur und AES-128-CBC-verschlüsseltem Body allein über `node:crypto` — die Zugangsdaten verlassen das Backend nie, und die Session wird gecacht (Reconnect bei Ablauf, wie bei UniFi lokal).
